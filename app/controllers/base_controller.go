...
func (server *Server) GetProvinces() ([]models.Province, error) {
	// Ensure the base URL includes a protocol scheme (http/https)
	baseURL := os.Getenv("API_ONGKIR_BASE")
	if !strings.HasPrefix(baseURL, "http") {
		baseURL = "https://" + baseURL
	}

	// Construct the full URL
	url := baseURL + "province?key=" + os.Getenv("API_ONGKIR_KEY")
	fmt.Println("Requesting provinces from:", url) // Debugging line
...
func (server *Server) GetCitiesByProvinceID(provinceID string) ([]models.City, error) {
	...
	// Construct the full URL
	url := baseURL + "city?key=" + os.Getenv("API_ONGKIR_KEY") + "&province=" + provinceID
	fmt.Println("Requesting cities for province:", url) // Debugging line
...
func (server *Server) CalculateShippingFee(shippingParams models.ShippingFeeParams) ([]models.ShippingFeeOption, error) {
	...
	response, err := http.PostForm(os.Getenv("API_ONGKIR_BASE")+"cost", params)
	if err != nil {
		log.Fatal(err)
		return nil, err
	}
